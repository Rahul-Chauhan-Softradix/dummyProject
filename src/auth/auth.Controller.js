...
import { forgetPassword } from '../EmailTemplates/forgotPassword'
...
export default class Auth {
  ...
  async userRegistration(req, res) {
    ...
    if (checkUserEmail) {
      return res.send(errorResponse(CUSTOM_MESSAGES.EMAIL_EXIST, null, RESPONSE_CODES.POST));
    }
    /** check user phone_number */
    let checkPhone = await this.services.getByPhone(phone_number);
    if (checkPhone) {
      return res.send(errorResponse(CUSTOM_MESSAGES.PHONE_NUMBER_EXIST, null, RESPONSE_CODES.POST));
    }
    ...
    const htmlContent = await forgetPassword()
  ...
  async userLogin(req, res) {
    ...
    if (!checkEmail) {
      return res.send(errorResponse(CUSTOM_MESSAGES.INVALID_EMAIL, null, RESPONSE_CODES.POST));
    }
    /** check user password */
    const checkPassword = await bcrypt.compare(password, checkEmail.password);
    if (!checkPassword) {
      return res.send(errorResponse(CUSTOM_MESSAGES.INVALID_PASSWORD, null, RESPONSE_CODES.POST));
    }
  ...
  async resetPassword(req, res) {
    ...
    res.send(successResponse(CUSTOM_MESSAGES.PASSWORD_UPDATED, null, RESPONSE_CODES.POST))
